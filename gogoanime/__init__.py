...
def gogo_auth(email, password):
    s = requests.session()
    animelink = "https://gogoanime.llc/login.html"
...
def get_search_results(query, page = 0):
        animelink = f"https://gogoanime.llc/search.html?keyword={query}&page={page}"
...
def get_anime_newseason(page = 0):
        animelink = f"https://gogoanime.llc/new-season.html?page={page}"
        response = requests.get(animelink)
        response_html = response.text
        soup = BeautifulSoup(response_html, 'html.parser')
        source_url = soup.find('div', 'last_episodes').find('ul', 'items').find_all('li')
...
def get_anime_popular(page = 0):
        animelink = f"https://gogoanime.llc/popular.html?page={page}"
        response = requests.get(animelink)
        response_html = response.text
        soup = BeautifulSoup(response_html, 'html.parser')
        source_url = soup.find('div', 'last_episodes').find('ul', 'items').find_all('li')
...
def get_anime_details(id):  
        animelink = f"https://gogoanime.llc/category/{id}"
        response = requests.get(animelink)
...
def get_anime_episode(email, password, id, episode):
    ...
    if auth_gogo:
        res_episode_list = []
        animelink = requests.get(f"https://gogoanime.llc/{id}-episode-{episode}", cookies=dict(auth=auth_gogo))      
